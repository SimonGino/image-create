...
**Mode（模式）**:
Generation 的两种形态之一：文生图（t2i）或参考图生图（reference）。由请求是否携带参考图决定，不是用户单独选择的开关。
...
可收藏、可复用的提示词，附默认模型；可附一张封面图（取自某次生成结果，指向的图被删后模板照常存在）。
_Avoid_: 示例、精选（模板就是用户自己的，没有第二套"官方示例"概念）
